...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView
{
    ...
    public NestedScrollView(final Context context, final AttributeSet set, final int n) {
        ...
        ViewCompat.setAccessibilityDelegate((View)this, (AccessibilityDelegateCompat)NestedScrollView.ACCESSIBILITY_DELEGATE);
    }
    ...
    public boolean onTouchEvent(final MotionEvent motionEvent) {
        ...
        switch (actionMasked) {
            ...
            case 2: {
                ...
                if (this.dispatchNestedScroll(0, n5, 0, n4 - n5, this.mScrollOffset, 0)) {
                ...
                if (n6 < 0) {
                    EdgeEffectCompat.onPull(this.mEdgeGlowTop, n4 / (float)this.getHeight(), motionEvent.getX(pointerIndex) / this.getWidth());
                    if (!this.mEdgeGlowBottom.isFinished()) {
                        this.mEdgeGlowBottom.onRelease();
                    }
                }
                else if (n6 > scrollRange) {
                    EdgeEffectCompat.onPull(this.mEdgeGlowBottom, n4 / (float)this.getHeight(), 1.0f - motionEvent.getX(pointerIndex) / this.getWidth());
                    if (!this.mEdgeGlowTop.isFinished()) {
                        this.mEdgeGlowTop.onRelease();
                    }
                }
