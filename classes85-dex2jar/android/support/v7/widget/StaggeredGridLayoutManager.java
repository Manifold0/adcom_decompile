...
public class StaggeredGridLayoutManager extends LayoutManager implements ScrollVectorProvider
{
    ...
    private int fill(final Recycler recycler, final LayoutState layoutState, final State state) {
        ...
        if (this.mLayoutState.mLayoutDirection == -1) {
            n12 = this.mPrimaryOrientation.getStartAfterPadding() - this.getMinStart(this.mPrimaryOrientation.getStartAfterPadding());
        }
        else {
            n12 = this.getMaxEnd(this.mPrimaryOrientation.getEndAfterPadding()) - this.mPrimaryOrientation.getEndAfterPadding();
        }
        if (n12 > 0) {
            return Math.min(layoutState.mAvailable, n12);
        }
        return 0;
    }
            ...
            for (int i = 0; i < childCount; ++i) {
                ...
                if (n >= max) {
                    ...
                    if (((LayoutParams)child.getLayoutParams()).isFullSpan()) {
                        n2 = 1.0f * n / this.mSpanCount;
                    }
                    max = Math.max(max, n2);
                }
            }
            final int mSizePerSpan = this.mSizePerSpan;
            int n3 = Math.round(this.mSpanCount * max);
            if (this.mSecondaryOrientation.getMode() == Integer.MIN_VALUE) {
                n3 = Math.min(n3, this.mSecondaryOrientation.getTotalSpace());
            }
                ...
                for (int j = 0; j < childCount; ++j) {
                    ...
                    if (!layoutParams.mFullSpan) {
                        if (this.isLayoutRTL() && this.mOrientation == 1) {
                            ...
                            final int n4 = layoutParams.mSpan.mIndex * this.mSizePerSpan;
                            final int n5 = layoutParams.mSpan.mIndex * mSizePerSpan;
                            if (this.mOrientation == 1) {
                                child2.offsetLeftAndRight(n4 - n5);
                            }
                            else {
                                child2.offsetTopAndBottom(n4 - n5);
                            }
                        }
                    }
                }
    ...
    public void collectAdjacentPrefetchPositions(int n, int i, final State state, final LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        if (((RecyclerView.LayoutManager)this).getChildCount() != 0 && n != 0) {
            ...
            int n2;
            int n3;
            for (i = 0; i < this.mSpanCount; ++i, n = n3) {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(final Recycler recycler, final State state, final View view, final AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
        ...
        if (this.mOrientation == 0) {
            ...
            accessibilityNodeInfoCompat.setCollectionItemInfo((Object)AccessibilityNodeInfoCompat$CollectionItemInfoCompat.obtain(spanIndex, mSpanCount, -1, -1, layoutParams2.mFullSpan, false));
            return;
        }
        ...
        accessibilityNodeInfoCompat.setCollectionItemInfo((Object)AccessibilityNodeInfoCompat$CollectionItemInfoCompat.obtain(-1, -1, spanIndex2, mSpanCount2, layoutParams2.mFullSpan, false));
    }
