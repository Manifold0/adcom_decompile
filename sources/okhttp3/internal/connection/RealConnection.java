...
public final class RealConnection extends Listener implements Connection {
    ...
    private Request createTunnel(int readTimeout, int writeTimeout, Request tunnelRequest, HttpUrl url) throws IOException {
        ...
        } while (!String.CLOSE.equalsIgnoreCase(response.header("Connection")));
    ...
    public String toString() {
        return "Connection{" + this.route.address().url().host() + ":" + this.route.address().url().port() + ", proxy=" + this.route.proxy() + " hostAddress=" + this.route.socketAddress() + " cipherSuite=" + (this.handshake != null ? this.handshake.cipherSuite() : ParametersKeys.ORIENTATION_NONE) + " protocol=" + this.protocol + '}';
    }
}
