    ...
    private static void validateOpenGraphKey(String key, boolean requireNamespace) {
        if (requireNamespace) {
            ...
            if (components.length < 2) {
                throw new FacebookException("Open Graph keys must be namespaced: %s", new Object[]{key});
            }
            for (String component : components) {
                if (component.isEmpty()) {
                    throw new FacebookException("Invalid key found in Open Graph dictionary: %s", new Object[]{key});
                }
            }
        }
    }
