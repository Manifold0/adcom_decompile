...
final class zzacb implements zzalz<zzon> {
    ...
    private final zzon zzd(InputStream inputStream) {
        ...
        if (PlatformVersion.isAtLeastKitKat() && zzakb.zzqp()) {
            ...
            zzakb.m3915v("Decoded image w: " + width + " h:" + height + " bytes: " + decodeStream.getAllocationByteCount() + " time: " + (uptimeMillis2 - uptimeMillis) + " on ui thread: " + (Looper.getMainLooper().getThread() == Thread.currentThread()));
        }
