...
final class zzf implements zzt {
    ...
    private final synchronized boolean zzb(zzr<?> zzr) {
        ...
        synchronized (this) {
            ...
            if (this.zzp.containsKey(url)) {
                ...
                if (zzaf.DEBUG) {
                    zzaf.m3912d("Request for cacheKey=%s is in flight, putting on hold.", url);
                }
                z = true;
            } else {
                ...
                zzr.zza((zzt) this);
                if (zzaf.DEBUG) {
                    zzaf.m3912d("new request, sending to network %s", url);
                }
            }
        }
        return z;
    ...
    public final synchronized void zza(zzr<?> zzr) {
        ...
        if (!(list == null || list.isEmpty())) {
            if (zzaf.DEBUG) {
                zzaf.m3914v("%d waiting requests for cacheKey=%s; resend to network", Integer.valueOf(list.size()), url);
            }
            ...
            try {
                ...
                zzaf.m3913e("Couldn't add request to queue. %s", e.toString());
    ...
    public final void zza(zzr<?> zzr, zzx<?> zzx) {
        ...
        if (list != null) {
            if (zzaf.DEBUG) {
                zzaf.m3914v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(list.size()), url);
            }
            for (zzr zzb : list) {
