...
public final class BlockingObservable<T> {
    ...
    /* renamed from: o */
    private final Observable<? extends T> f8389o;

    /* renamed from: rx.observables.BlockingObservable$2 */
    class C33112 implements Iterable<T> {
        C33112() {
        }
    ...
    class C33178 implements Action1<Throwable> {
        C33178() {
        }
    ...
    public void forEach(final Action1<? super T> onNext) {
        ...
        BlockingUtils.awaitForComplete(latch, this.f8389o.subscribe(new Subscriber<T>() {
    ...
    public Iterator<T> getIterator() {
        return BlockingOperatorToIterator.toIterator(this.f8389o);
    }

    public T first() {
        return blockForSingle(this.f8389o.first());
    }

    public T first(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f8389o.first(predicate));
    }

    public T firstOrDefault(T defaultValue) {
        return blockForSingle(this.f8389o.map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T firstOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f8389o.filter(predicate).map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T last() {
        return blockForSingle(this.f8389o.last());
    }

    public T last(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f8389o.last(predicate));
    }

    public T lastOrDefault(T defaultValue) {
        return blockForSingle(this.f8389o.map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public T lastOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f8389o.filter(predicate).map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public Iterable<T> mostRecent(T initialValue) {
        return BlockingOperatorMostRecent.mostRecent(this.f8389o, initialValue);
    }

    public Iterable<T> next() {
        return BlockingOperatorNext.next(this.f8389o);
    }

    public Iterable<T> latest() {
        return BlockingOperatorLatest.latest(this.f8389o);
    }

    public T single() {
        return blockForSingle(this.f8389o.single());
    }

    public T single(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f8389o.single(predicate));
    }

    public T singleOrDefault(T defaultValue) {
        return blockForSingle(this.f8389o.map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public T singleOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f8389o.filter(predicate).map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public Future<T> toFuture() {
        return BlockingOperatorToFuture.toFuture(this.f8389o);
    }
    ...
    public void subscribe() {
        ...
        BlockingUtils.awaitForComplete(cdl, this.f8389o.subscribe(new Subscriber<T>() {
    ...
    public void subscribe(Observer<? super T> observer) {
        ...
        Subscription s = this.f8389o.subscribe(new Subscriber<T>() {
            public void onNext(T t) {
    ...
    public void subscribe(Action1<? super T> onNext) {
        subscribe(onNext, new C33178(), Actions.empty());
    }
