...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new C26451();

    /* renamed from: android.support.v7.util.DiffUtil$1 */
    static class C26451 implements Comparator<Snake> {
        C26451() {
        }

        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f6602x - o2.f6602x;
            return cmpX == 0 ? o1.f6603y - o2.f6603y : cmpX;
        }
    }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f6602x != 0 || firstSnake.f6603y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f6602x + snake.size;
                int endY = snake.f6603y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f6602x + j;
                    int newItemPos = snake.f6603y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f6602x + snakeSize;
                int endY = snake.f6603y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f6602x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f6602x + i, 1, this.mCallback.getChangePayload(snake.f6602x + i, snake.f6603y + i));
                    }
                }
                posOld = snake.f6602x;
                posNew = snake.f6603y;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f6602x += range.oldListStart;
                snake.f6603y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f6602x;
                    left.newListEnd = snake.f6603y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f6602x - 1;
                    left.newListEnd = snake.f6603y;
                } else {
                    left.oldListEnd = snake.f6602x;
                    left.newListEnd = snake.f6603y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f6602x + snake.size;
                    right.newListStart = snake.f6603y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = (snake.f6602x + snake.size) + 1;
                    right.newListStart = snake.f6603y + snake.size;
                } else {
                    right.oldListStart = snake.f6602x + snake.size;
                    right.newListStart = (snake.f6603y + snake.size) + 1;
                }
                stack.add(right);
            } else {
                rangePool.add(range);
        ...
        while (d <= dLimit) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f6602x = backward[kOffset + k];
                    outSnake.f6603y = outSnake.f6602x - k;
            ...
            while (k <= d) {
                ...
                if (checkInFwd || k + delta < (-d) || k + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake = new Snake();
                    outSnake.f6602x = backward[kOffset + backwardK];
                    outSnake.f6603y = outSnake.f6602x - backwardK;
