...
public class DownloadFileTask extends AsyncTask<Void, Integer, Long> {
    ...
    protected void setConnectionProperties(HttpURLConnection connection) {
        ...
        if (VERSION.SDK_INT <= 9) {
            connection.setRequestProperty("connection", String.CLOSE);
        }
    }
    ...
    protected URLConnection createConnection(URL url, int remainingRedirects) throws IOException {
        ...
        if ((code != 301 && code != IronSourceConstants.OFFERWALL_AVAILABLE && code != 303) || remainingRedirects == 0) {
            return connection;
        }
    ...
    protected void onPostExecute(Long result) {
        ...
        try {
            ...
            builder.setTitle(C0982R.string.hockeyapp_download_failed_dialog_title);
            if (this.mDownloadErrorMessage == null) {
                message = this.mContext.getString(C0982R.string.hockeyapp_download_failed_dialog_message);
            } else {
                message = this.mDownloadErrorMessage;
            }
            builder.setMessage(message);
            builder.setNegativeButton(C0982R.string.hockeyapp_download_failed_dialog_negative_button, new C00451());
            builder.setPositiveButton(C0982R.string.hockeyapp_download_failed_dialog_positive_button, new C00462());
            builder.create().show();
        } catch (Exception e2) {
        }
    }
