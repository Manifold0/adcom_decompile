...
public class CheckUpdateTaskWithUI extends CheckUpdateTask {
    ...
    class C00432 implements OnCancelListener {
        C00432() {
        }
    ...
    private void showDialog(final JSONArray updateInfo) {
        ...
        if (this.mActivity != null && !this.mActivity.isFinishing()) {
            ...
            builder.setTitle(C0982R.string.hockeyapp_update_dialog_title);
            if (this.mandatory.booleanValue()) {
                String appName = Util.getAppName(this.mActivity);
                Toast.makeText(this.mActivity, String.format(this.mActivity.getString(C0982R.string.hockeyapp_update_mandatory_toast), new Object[]{appName}), 1).show();
                startUpdateIntent(updateInfo, Boolean.valueOf(true));
                return;
            }
            builder.setMessage(C0982R.string.hockeyapp_update_dialog_message);
            builder.setNegativeButton(C0982R.string.hockeyapp_update_dialog_negative_button, new C00421());
            builder.setOnCancelListener(new C00432());
            builder.setPositiveButton(C0982R.string.hockeyapp_update_dialog_positive_button, new OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
