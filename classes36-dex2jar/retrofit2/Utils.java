...
final class Utils
{
    ...
    static boolean hasUnresolvableType(final Type type) {
        if (!(type instanceof Class)) {
            if (type instanceof ParameterizedType) {
    ...
    private static final class ParameterizedTypeImpl implements ParameterizedType
    {
        ...
        ParameterizedTypeImpl(final Type ownerType, Type rawType, final Type... array) {
            int n = 1;
            final int n2 = 0;
            if (rawType instanceof Class) {
                int n3;
            ...
            for (int length = typeArguments.length, i = n2; i < length; ++i) {
                rawType = typeArguments[i];
                if (rawType == null) {
                    throw new NullPointerException();
                }
                Utils.checkNotPrimitive(rawType);
            }
        }
