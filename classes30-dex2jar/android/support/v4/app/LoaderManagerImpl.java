...
class LoaderManagerImpl extends LoaderManager
{
    ...
    LoaderManagerImpl(final String mWho, final FragmentHostCallback mHost, final boolean mStarted) {
        this.mLoaders = (SparseArrayCompat<LoaderInfo>)new SparseArrayCompat();
        this.mInactiveLoaders = (SparseArrayCompat<LoaderInfo>)new SparseArrayCompat();
        this.mWho = mWho;
        this.mHost = mHost;
    ...
    public void destroyLoader(int indexOfKey) {
        ...
        if (indexOfKey2 >= 0) {
            final LoaderInfo loaderInfo = (LoaderInfo)this.mLoaders.valueAt(indexOfKey2);
            this.mLoaders.removeAt(indexOfKey2);
            loaderInfo.destroy();
        }
        indexOfKey = this.mInactiveLoaders.indexOfKey(indexOfKey);
        if (indexOfKey >= 0) {
            final LoaderInfo loaderInfo2 = (LoaderInfo)this.mInactiveLoaders.valueAt(indexOfKey);
    ...
    void doDestroy() {
        ...
        for (int j = this.mInactiveLoaders.size() - 1; j >= 0; --j) {
            ((LoaderInfo)this.mInactiveLoaders.valueAt(j)).destroy();
        }
    ...
    public void dump(final String s, final FileDescriptor fileDescriptor, final PrintWriter printWriter, final String[] array) {
        if (this.mLoaders.size() > 0) {
            ...
            for (int i = 0; i < this.mLoaders.size(); ++i) {
                final LoaderInfo loaderInfo = (LoaderInfo)this.mLoaders.valueAt(i);
                printWriter.print(s);
                printWriter.print("  #");
        ...
        if (this.mInactiveLoaders.size() > 0) {
            ...
            for (int j = 0; j < this.mInactiveLoaders.size(); ++j) {
                final LoaderInfo loaderInfo2 = (LoaderInfo)this.mInactiveLoaders.valueAt(j);
                printWriter.print(s);
                printWriter.print("  #");
    ...
    public <D> Loader<D> getLoader(final int n) {
        ...
        final LoaderInfo loaderInfo = (LoaderInfo)this.mLoaders.get(n);
        if (loaderInfo == null) {
            return null;
        }
    ...
    void installLoader(final LoaderInfo loaderInfo) {
        this.mLoaders.put(loaderInfo.mId, (Object)loaderInfo);
        if (this.mStarted) {
            loaderInfo.start();
        }
    }
    ...
    public <D> Loader<D> restartLoader(final int n, final Bundle bundle, final LoaderCallbacks<D> loaderCallbacks) {
        ...
        if (loaderInfo != null) {
            final LoaderInfo loaderInfo2 = (LoaderInfo)this.mInactiveLoaders.get(n);
            if (loaderInfo2 != null) {
                if (loaderInfo.mHaveData) {
                    ...
                    this.mInactiveLoaders.put(n, (Object)loaderInfo);
                }
                else {
                    ...
                    this.mLoaders.put(n, (Object)null);
                    loaderInfo.destroy();
                }
            }
            else {
    ...
    final class LoaderInfo implements Loader$OnLoadCompleteListener<Object>, Loader$OnLoadCanceledListener<Object>
    {
                ...
                try {
                    ...
                    if (this.mLoader != null) {
                        if (this.mListenerRegistered) {
                            ...
                            this.mLoader.unregisterOnLoadCanceledListener((Loader$OnLoadCanceledListener)this);
                        }
                        this.mLoader.reset();
                    }
        ...
        public void onLoadComplete(final Loader<Object> loader, final Object mData) {
            ...
            if (this.mDestroyed) {
                ...
                final LoaderInfo loaderInfo = (LoaderInfo)LoaderManagerImpl.this.mInactiveLoaders.get(this.mId);
        ...
        void start() {
            if (this.mRetaining && this.mRetainingStarted) {
                ...
                if (this.mLoader != null) {
                    ...
                    if (!this.mListenerRegistered) {
                        this.mLoader.registerListener(this.mId, (Loader$OnLoadCompleteListener)this);
                        this.mLoader.registerOnLoadCanceledListener((Loader$OnLoadCanceledListener)this);
        ...
        public String toString() {
            ...
            sb.append(this.mId);
            sb.append(" : ");
            DebugUtils.buildShortClassTag((Object)this.mLoader, sb);
            sb.append("}}");
            return sb.toString();
        }
    }
