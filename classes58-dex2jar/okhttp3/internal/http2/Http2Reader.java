...
final class Http2Reader implements Closeable
{
    ...
    public void readConnectionPreface(final Handler handler) throws IOException {
        if (this.client) {
            ...
            final ByteString byteString = this.source.readByteString((long)Http2.CONNECTION_PREFACE.size());
            if (Http2Reader.logger.isLoggable(Level.FINE)) {
                Http2Reader.logger.fine(Util.format("<< CONNECTION %s", byteString.hex()));
            }
            if (!Http2.CONNECTION_PREFACE.equals((Object)byteString)) {
